...
### 1. Unresolved review thread (bot: @chatgpt-codex-connector) in `projects/project-01-platform-core/operator/reconcile.py:76`

> **<sub><sub>![P2 Badge](https://img.shields.io/badge/P2-yellow?style=flat)</sub></sub>  Populate ownerReference UID from object metadata**

When `desired_job()` is used for any real TrainingRun, this reads `ownerUid` from `status`, but the CRD status schema does not define that field and the operator never writes it, so the generated Job carries an empty ownerReference UID. Kubernetes requires owner references to include the owning object's metadata UID; with an empty UID the Job create/apply is

### 2. Unresolved review thread (bot: @chatgpt-codex-connector) in `projects/project-01-platform-core/control-plane/openapi.yaml:168`
